...
public class WeatherDriver extends Configured implements Tool {
	...
	public static void main(String[] args) {
		...
		if(args.length<2) {
			System.out.println("Usage Driver Class : "+WeatherDriver.class.getName()+" input HDFS /Destination/path output HDFS /Destination/path ");
			return;
		}
		...
		try {
			ToolRunner.run(configuration, new WeatherDriver(), args);
		} catch (Exception e) {
	...
	public int run(String[] args) throws Exception {
		...
		Job weatherCountJob = Job.getInstance(configuration, WeatherDriver.class.getName());
		
		weatherCountJob.setJarByClass(WeatherDriver.class);
		
		String input =args[0];
		Path hdfsInput = new Path(input);
		TextInputFormat.addInputPath(weatherCountJob, hdfsInput);
		
		String output =args[1];
		Path hdfsOutput = new Path(output);
		TextOutputFormat.setOutputPath(weatherCountJob, hdfsOutput);
		
		weatherCountJob.setMapperClass(WeatherMapper.class);
		
		weatherCountJob.setReducerClass(WeatherReducer.class);
		
		weatherCountJob.setMapOutputKeyClass(Text.class);
		weatherCountJob.setMapOutputValueClass(IntWritable.class);
		
		weatherCountJob.setOutputKeyClass(Text.class);
		weatherCountJob.setOutputValueClass(LongWritable.class);
		...
		*/
		...
		return weatherCountJob.waitForCompletion(true) ? 0 : 1;
	}
